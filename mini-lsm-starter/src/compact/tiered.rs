...
impl TieredCompactionController {
    ...
    pub fn generate_compaction_task(
        ...
        // space amp ratio
        let spc_amp_ratio =  _snapshot.levels.iter()
            .map(|x| x.1.len()).sum::<usize>() as f64
            / _snapshot.levels.last().unwrap().1.len() as f64  - 1_f64;
        ...
        for i in 0.._snapshot.levels.len() - 1 {
            ...
            if pre_sz as f64 / _snapshot.levels[i + 1].1.len() as f64 >= sz_ratio_trigger {
                return Some(TieredCompactionTask{tiers: Vec::from(&_snapshot.levels[0..=(i+1)]), bottom_tier_included: (i == _snapshot.levels.len() - 2)});
            }
        }
        ...
        return Some(TieredCompactionTask{
            tiers: _snapshot.levels.iter().take(num_tiers_to_take)
    ...
    pub fn apply_compaction_result(
        ...
        _output: &[usize],
    ) -> (LsmStorageState, Vec<usize>) {  // in state lock
        let mut new_state = _snapshot.clone();
        let mut old_hash_ssts = _task.tiers.iter()
            .map(|(x, y)| (*x, y))
            .collect::<HashMap<_, _>>();
