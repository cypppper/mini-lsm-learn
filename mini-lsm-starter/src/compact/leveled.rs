...
impl LeveledCompactionController {
    ...
    fn find_overlapping_ssts(
        ...
        let upper_min_key = _sst_ids.iter().map(|id| {
            _snapshot.sstables.get(id).unwrap().first_key()
        }).min().unwrap();
        let upper_max_key = _sst_ids.iter().map(|id| {
            _snapshot.sstables.get(id).unwrap().last_key()
        }).max().unwrap();
        
        let lower_len = _snapshot.levels[_in_level - 1].1.len();
        let first_id = _snapshot.levels[_in_level - 1].1.partition_point(|id| {
            _snapshot.sstables.get(id).unwrap().last_key() < upper_min_key
        });
        if first_id == lower_len {  // no overlap
            return vec![];
        }
        let last_id = _snapshot.levels[_in_level - 1].1.partition_point(|id| {
            _snapshot.sstables.get(id).unwrap().first_key() <= upper_max_key
        });
        if last_id == 0 {
            return vec![];
        }
    ...
    pub fn generate_compaction_task(
        ...
        if _snapshot.l0_sstables.len() >= self.options.level0_file_num_compaction_trigger {
            ...
            if level_sz[valid_idx] >= self.options.base_level_size_mb * 1024 * 1024 && valid_idx != 0 {
                return Some(LeveledCompactionTask {
                    upper_level: None,
                    upper_level_sst_ids: _snapshot.l0_sstables.clone(),
                    lower_level: valid_idx,
            ...
            return Some(LeveledCompactionTask {
                ...
                lower_level_sst_ids: self.find_overlapping_ssts(_snapshot,& _snapshot.l0_sstables, valid_idx + 1),
                is_lower_level_bottom_level: valid_idx + 1 == level_sz.len(),
            });
        }
        if valid_idx >= level_sz.len() - 1 {return None;}
        let level_sz = level_sz.into_iter().map(|x| (x/ 1024/ 1024) as f64).collect::<Vec<_>>();  // mb & valid_idx
        println!("level sz: {:?}", level_sz);
        let mut target_sz = if *level_sz.last().unwrap() > self.options.base_level_size_mb as f64 {*level_sz.last().unwrap()} else  {self.options.base_level_size_mb as f64};
    ...
    pub fn apply_compaction_result(
        ...
        let mut old_upper_ids = _task.upper_level_sst_ids.iter().copied().collect::<HashSet<_>>();
        let mut old_lower_ids = _task.lower_level_sst_ids.iter().copied().collect::<HashSet<_>>();
        if let Some(lv) = _task.upper_level {
            new_state.levels[lv - 1].1.retain(|id| !old_upper_ids.remove(&id));
        } else {
            new_state.l0_sstables.retain(|id| !old_upper_ids.remove(&id));
        }
        new_state.levels[_task.lower_level - 1].1.retain(|id| !old_lower_ids.remove(id));

        // insert new sst
        new_state.levels[_task.lower_level - 1].1.extend_from_slice(_output);
        new_state.levels[_task.lower_level - 1].1.sort_by(|a,b| {
            _snapshot.sstables.get(a).unwrap().first_key().cmp(
                _snapshot.sstables.get(b).unwrap().first_key()
            )
        });
        let mut del_ids = vec![];
